...
#define LI_BIT 3  // src-BF, dst-BF 비트 (individual)
#define LT_BIT 4  // tuple-BF 비트
#define LH_BIT 3  // hash table 비트

#define IP_SIZE 4   // IP주소 크기 
#define RN 6   // rule 개수
#define LI 1 << LI_BIT // src-BF, dst-BF 크기
#define LT 1 << LT_BIT // tuple-BF 크기
#define LH 1 << LH_BIT   // hash table 크기
#define CRC_SIZE 8  // CRC 코드 길이
#define N 100  // maximum BMR value
...
struct crc32 {
    ...
    static uint32_t update(uint32_t(&table)[256], uint32_t initial, const void* buf, size_t len)
    {
        ...
        for (size_t i = 0; i < len; ++i)
            c = table[(c ^ (u[i]-'0')) & 0xFF] ^ (c >> 8);
        return c ^ 0xFFFFFFFF;
    }
};
*/

uint32_t CRC_32(const string& input) {
...
int main()
{
    ...
    cout << hex << CRC_32("0") << endl;
    cout << hex << CRC_32("00") << endl;
    cout << hex << CRC_32("000") << endl;
    cout << hex << CRC_32("1") << endl; 
    cout << hex << CRC_32("01") << endl;
    cout << hex << CRC_32("001") << endl;
